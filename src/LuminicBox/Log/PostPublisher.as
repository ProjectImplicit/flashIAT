...
*/
class LuminicBox.Log.PostPublisher implements IPublisher {
	...
	*/
	function publish(e:LogEvent):Void {
		...
		poster.rawData="";
		poster.addResultValue("FLASH",txt);
		poster.sendAndLoad("/implicit/StudyLogs;jsessionid="+_root.JID, poster, "POST");
		poster.onData =  handleResponse;
		// time out error after 90 seconds
		var timeoutInterval = setInterval(handleResponse,90000);
	}
	
    private function handleResponse(src) {
    	if(src=="1"){
    		getURL("/implicit/research/TaskErrorRedirect.html?l=1","_self");
    	}
    	else{
    		getURL("/implicit/research/TaskErrorRedirect.html?l=0","_self");
    	}
    }
