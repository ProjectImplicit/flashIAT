		...
		try {
			if (!success) {
				...
				if (parentNode.nodeName == null) {
					throw new VideoError(VideoError.INVALID_XML, "URL: \"" + _url + "\" No root node found; if file is an flv it must have .flv extension");
				} else if (parentNode.nodeName.toLowerCase() != "smil") {
					throw new VideoError(VideoError.INVALID_XML, "URL: \"" + _url + "\" Root node not smil: " + parentNode.nodeName);
				}
	...
	private function parseSwitch(parentNode:XMLNode):Void {
		for (var i:Number = 0; i < parentNode.childNodes.length; i++) {
			...
			if (node.nodeName.toLowerCase() == "video" || node.nodeName.toLowerCase() == "ref") {
				var videoObj:Object = parseVideo(node);
				if (videoObj.bitrate == undefined) {
					videoTags.push(videoObj);
				} else {
					var inserted:Boolean = false;
					for (var j:Number = 0; j < videoTags.length; j++) {
						if (videoTags[j].bitrate == undefined || videoObj.bitrate < videoTags[i].bitrate) {
							inserted = true;
							videoTags.splice(j, 0, videoTags);
							break;
						}
					}
					if (!inserted) {
	...
	private function parseVideo(node:XMLNode):Object {
		...
		for (var attr:String in node.attributes) {
			if (attr.toLowerCase() == "src") {
				...
				// todo: parse time!
				obj.dur = Number(node.attributes[attr]);
			} else {
				//ifdef DEBUG
				//throw new VideoError(VideoError.INVALID_XML, "URL: \"" + _url + "\" Attribute " + attr + " not supported in " + node.nodeName + " tag.");
				//endif
